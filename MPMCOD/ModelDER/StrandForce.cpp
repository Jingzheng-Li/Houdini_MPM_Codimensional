...
StrandForce::StrandForce(const std::shared_ptr<SIMManager>& scene,
...
void StrandForce::updateStartState() {
	...
	for (int i = 0; i < getNumVertices(); ++i) {
		if (m_scene->isTip(m_verts[i]))
			currentStrandDoFs.segment<3>(i * 4) = m_scene->getX().segment<3>(m_scene->getDof(m_verts[i]));
		else
			currentStrandDoFs.segment<4>(i * 4) = m_scene->getX().segment<4>(m_scene->getDof(m_verts[i]));
	}
...
void StrandForce::addHessXToTotal(const VectorXs& x, const VectorXs& v,
	...
	threadutils::for_each(0, num_hess, [&](int i) {
		...
		hessE[hessE_index + i] =
			Triplets(4 * m_verts[row_vert] + row_r, 4 * m_verts[col_vert] + col_r, -data.value());
	});
}
